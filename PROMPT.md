Build a premium, internationally-positioned tech company website for Vrishtek —
a full-stack software product studio. The site must feel like it belongs alongside
Vercel, Linear, and Stripe in terms of design quality and engineering credibility.

═══════════════════════════════════════════
BRAND IDENTITY
═══════════════════════════════════════════
Company name:   Vrishtek
Tagline:        "Engineered with intent."
               (alternatives to A/B test: "Built to last." / "Systems that scale.")
Brand persona:  Premium Indian tech studio with international positioning.
               Not an outsourcing firm. Not a freelancer collective.
...
/contact          → Inquiry form + calendar booking
...
SECTION 1 - HERO
   ...
   Subheadline (1–2 lines, muted):
      "Vrishtek builds full-stack digital products for startups and
       scale-ups who refuse to ship mediocrity."
   Two CTAs:
...
SECTION 3 - SELECTED WORK (2–3 project previews)
   Each project card shows:
      ...
      - Stack tags (React / Node / PostgreSQL etc.)
      - Outcome metric if available ("Reduced load time by 60%",
        "Scaled to 10K users in 3 months", "Launched in 8 weeks")
      - Dark preview image or animated UI screenshot
...
SECTION 4 - TECH PHILOSOPHY (short, text-heavy - signals depth)
   ...
   Followed by: stack logo strip (small icons, 1 row)
...
SECTION 7 - FOOTER CTA
   ...
   Below: email address (hello@vrishtek.com) displayed plainly

FOOTER (global):
   Left:  Vrishtek logo + tagline
   Mid:   Nav links (Work / Services / Stack / Blog / Careers)
   Right: Social icons (GitHub / LinkedIn / Twitter/X)
   Bottom bar: © 2025 Vrishtek. Built with Next.js. Deployed on AWS.
...
Each card (alternating image side):
   ...
   - [Read full case study] → individual /work/[slug] page

Individual case study page structure:
   Overview → Problem → Our approach → Technical decisions (this is the differentiator —
   explain WHY you chose PostgreSQL over MongoDB for this project, why WebSockets
...
Services listed (each with its own section, not just bullets):
   ...
   1. Full-Stack Product Development
      "From greenfield to production. React/Next.js frontend, Node.js/Express
       backend, PostgreSQL + Prisma ORM, deployed on AWS ECS or Vercel."
      Best for: Startups building their first product / Series A companies
               rebuilding their MVP properly
   ...
   3. API Design & Backend Systems
      "REST and GraphQL APIs, event-driven systems with BullMQ + Redis,
       real-time features via WebSockets, and PostgreSQL schemas that
       don't become technical debt."
      Best for: Teams with a frontend but no reliable backend foundation

   4. React Native Mobile
      "Cross-platform iOS + Android from a single TypeScript codebase.
       Expo-managed workflow for speed, bare workflow when you need native modules."
      Best for: Web products expanding to mobile without a separate native team

   5. Technical Consulting & Architecture Review
      "2-week engagement. We review your current system, identify the 3 biggest
       risks, and give you a written remediation plan with prioritized actions."
      Best for: CTOs who inherited a codebase and need an outside read
...
Organized by layer:
   ...
   "Server components reduce client JS. App Router enables streaming and
    nested layouts that scale. We've been on this since beta."
   React 18 + TypeScript
   ...
   BACKEND
   Node.js + Express (or Fastify for high-throughput)
   ...
   PostgreSQL
   "Our default database. ACID compliance, JSONB when schema flexibility matters,
    PostGIS when location matters. We do not reach for MongoDB first."
   Prisma ORM
   "Schema-as-code. Migrations in version control. Type-safe queries.
    The query builder we would design ourselves if we had to."
   Redis
   ...
   MOBILE
   React Native + Expo
   "One codebase, two platforms. Expo managed workflow unless the project
    demands bare. We do not build separate native apps unless you have a
    10-person mobile team to maintain them."
   ...
   "CI/CD on every PR. Lint, type-check, test, build, deploy —
    automated from day one, not added six months later."
   Vercel
   "For Next.js frontends where Vercel's edge network is the right call.
    We choose between ECS and Vercel based on the project, not habit."

   AI INTEGRATION
   Anthropic Claude API
   "Our preferred LLM for text generation, summarization, and structured
    output. We prompt-engineer, version control prompts, and cap token spend."
   OpenAI API (when required)
   "For embeddings and cases where the client specifies GPT."
   LangChain (selectively)
   "We use it when the agent complexity justifies it. We do not add it
    to every AI feature just because it exists."

Page ends with honest note:
...
Team section (even solo: just Devang for now):
   Photo / avatar + name + title + 2-line bio + GitHub / LinkedIn links
   Honest note: "Vrishtek is currently a focused solo studio with
...
Form fields:
   ...
   - Estimated budget range (dropdown):
       Under $5K / $5K–$15K / $15K–$50K / $50K+ / Not sure yet
   - Timeline (dropdown):
       ASAP / 1–3 months / 3–6 months / Flexible
   - How did you find Vrishtek? (dropdown + optional text)
   - [Send inquiry →] button
...
Frontend:
   ...
   Framer Motion (page transitions, scroll-triggered reveals)
   shadcn/ui (for form components, dialogs)
   MDX (blog posts, via next-mdx-remote or @next/mdx)
...
Database:
   ...
   No Redis needed for a company website
...
Light mode:
   ...
   Text muted:      #71717A  (zinc-500)
   Accent:          #B8902A  (darker gold for contrast on white)
...
Typography:
   ...
   Import via: next/font/google (performance-optimized, no FOUT)
...
OG image:          Static for homepage (logo + tagline on dark bg)
                   Dynamic for blog posts (@vercel/og)
Sitemap:           /sitemap.xml (auto-generated via next-sitemap)
Robots:            /robots.txt (allow all, no noindex on public pages)
...
Target keywords (naturally worked into copy, not stuffed):
   "full-stack development studio India"
   "Next.js development agency"
   "MERN stack development company"
   "PostgreSQL Prisma development"
   "SaaS development India"
   "React Native app development studio"
   "hire full-stack engineer India"

═══════════════════════════════════════════
PERFORMANCE TARGETS
═══════════════════════════════════════════
Lighthouse scores (non-negotiable for a tech company's own site):
   Performance:    95+
   Accessibility:  95+
   Best Practices: 100
   SEO:            100
...
Techniques to achieve this:
   - next/image for all images (automatic WebP conversion + lazy load)
   - next/font for font loading (eliminates layout shift)
   - Static generation (generateStaticParams) for all blog posts
   - No third-party scripts except Plausible (async, deferred)
   - Critical CSS inlined (Tailwind + Next.js handles this automatically)
...
═══════════════════════════════════════════
Pre-launch:
   □ vrishtek.com domain registered (Namecheap or Cloudflare Registrar)
   □ hello@vrishtek.com email (Google Workspace $6/mo or Zoho Mail free)
   □ GitHub org: github.com/vrishtek (even 1 public repo = credibility)
   □ LinkedIn company page: linkedin.com/company/vrishtek
   □ Twitter/X: @vrishtek (reserve it even if not active)
   □ Neon.tech PostgreSQL instance provisioned
   □ Vercel project connected to GitHub repo (auto-deploy on push)
   □ Plausible or Umami analytics live
